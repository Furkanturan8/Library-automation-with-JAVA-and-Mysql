...
public class UpdateAdminGUI extends JFrame {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    UpdateAdminGUI frame = new UpdateAdminGUI(admin);
     ...
     * Create the frame.
     */
    public UpdateAdminGUI(Admin admin) {
        setTitle("Personel Güncelleme Ekranı");
        setResizable(false);
        ...
        JLabel lblPersonelGncellemeEkran = new JLabel("Kişisel Verileri Değiştirme Ekranı");
        lblPersonelGncellemeEkran.setBackground(new Color(255, 255, 255));
        lblPersonelGncellemeEkran.setBounds(13, 10, 584, 50);
        lblPersonelGncellemeEkran.setHorizontalAlignment(SwingConstants.CENTER);
        lblPersonelGncellemeEkran.setForeground(new Color(255, 255, 255));
        lblPersonelGncellemeEkran.setFont(new Font("Segoe UI Semilight", Font.BOLD | Font.ITALIC, 20));
        contentPane.add(lblPersonelGncellemeEkran);
        ...
        btnGoster.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if(admin.getPassword().equals(passwordField.getText())) {
                    ...
                    txtF_name.setText(admin.getName());
                    txtF_kAdi.setText(admin.getKullaniciAdi());
                    txtF_tc.setText(admin.getTc());
                    txtF_telNo.setText(admin.getTelefonNo());
                ...
                if(txtF_name.getText().length() == 0 || txtF_kAdi.getText().length() == 0 || txtF_tc.getText().length() == 0 || txtF_telNo.getText().length() == 0 || passwordF.getText().length()==0 || passwordFAgain.getText().length()==0) {
                    ...
                    try {
                        ...
                        if(passwordControl == 1) {
                            control = admin.updateAdmin(admin.getId(),txtF_kAdi.getText(),passwordF.getText(),txtF_name.getText(),txtF_tc.getText(), txtF_telNo.getText());
                            if(control == true) {
